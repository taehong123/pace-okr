...
export const themeBootstrapScript = `(() => {
  ...
  try {
    const saved = window.localStorage.getItem(${JSON.stringify(THEME_STORAGE_KEY)})
      || window.localStorage.getItem(${JSON.stringify(LEGACY_THEME_STORAGE_KEY)});
    if (Object.prototype.hasOwnProperty.call(schemes, saved)) theme = saved;
    if (saved && !window.localStorage.getItem(${JSON.stringify(THEME_STORAGE_KEY)})) {
      window.localStorage.setItem(${JSON.stringify(THEME_STORAGE_KEY)}, saved);
    }
  } catch {}
