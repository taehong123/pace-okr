...
export const WORK_FIELDS = {
  ...
  project: {
    ...
    tool: "propose_project → 대화에서 최종 내용·연결 승인 → confirm_project; 후보 검색·결과 확인은 get_project_review, 취소는 cancel_project_review",
  },
  routine: {
    required: ["title"],
...
export const WORKFLOW_INSTRUCTIONS = [
  ...
  "PROJECT APPROVAL IS MANDATORY and overrides reviewBeforeCreate and conflicting workspace defaults: a generic creation request does not authorize picking an Initiative. Read Initiative descriptions and their KR/Objective context. Recommend at most 3 only with concrete contribution reasons, not recency or vague keywords. Present title/scope, owners, deadline, every defaulted/provided property and recommended paths using propose_project. The user chooses and confirms in this conversation; accept edits here and call confirm_project with the complete final proposal, review version, current editor revision and chosen Initiative fingerprint. Do not require a browser visit or a separate web approval. Never fabricate consent, select the first/only parent automatically, add unseen fields at save time, or bypass review using another API/kind. If the user already explicitly approved this exact proposal and connection, do not ask the same confirmation again. A clear Task may still use General.",
  "Use create_tasks once for explicitly supplied Tasks sharing a container and common fields. Use create_item for non-Project items and propose_project then confirm_project for Projects. Never generate extra Tasks. Routine children use routine_id. Children inherit the selected parent's cycle_id. A pending/failed review is NOT a created Project. get_project_review(include_context=true) searches candidates and returns editable options without discarding draft edits; confirm_project returns the saved receipt and needs no web follow-up. On an uncertain save, read its receipt or repeat the identical confirmation, never make another proposal. cancel_project_review cancels a pending draft in this conversation.",
