...
type IdentityRow = { id: string; name: string; address: string | null; revision: number; role: string };

export async function readWorkspaceIdentity(db: D1Database, workspaceId: string, userId: string, subdomainsEnabled = false) {
  const row = await db.prepare(`SELECT w.id, w.name, s.address, COALESCE(s.revision, 0) AS revision, m.role
...
export async function updateWorkspaceIdentity(db: D1Database, workspaceId: string, userId: string, input: Record<string, unknown>, subdomainsEnabled = false) {
  ...
  const hasName = Object.hasOwn(input, "name"), hasAddress = Object.hasOwn(input, "address");
  if (!hasName && !hasAddress) throw new WorkspaceIdentityError("변경할 이름 또는 주소를 입력해 주세요.");
  ...
  if (name === current.name && address === current.address) return current;
  ...
  statements.push(
    db.prepare("UPDATE workspaces SET name = ?, updated_at = ? WHERE id = ?").bind(name, now, workspaceId),
