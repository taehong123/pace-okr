...
export function getLandingCopy(translate: (key: string) => string): LandingCopy {
  return {
    ...
    loginNote: translate("기존 계정은 로그인, 처음이면 회원가입으로 이어집니다."),
    loginError: translate("Google 로그인을 완료하지 못했습니다. 다시 시도해 주세요."),
    unavailable: translate("Google 로그인 설정을 완료하는 중입니다."),
    sample: translate("가상 데이터로 구성한 실제 제품 화면"),
    google: translate("Google의 OKR 이야기"), intel: translate("Intel의 OKR 사례"),
    mcp: translate("MCP를 연결하면 사용 중인 AI 대화에서도 이어갈 수 있습니다."),
    slack: translate("연결 버튼을 누르고 Slack에서 승인하면 멤버와 채널을 불러옵니다. 알림을 받을 대상과 시간을 설정해 시작하세요."),
    slides: [
      {
        title: translate("세계적인 기업들이 선택한 목표 관리 방식, OKR."),
        description: translate("구글과 인텔이 활용해 온 OKR. 이제 우리 팀의 일하는 방식으로 만드세요."),
        alt: translate("고객 경험 개선 목표와 측정 가능한 핵심결과를 함께 보여주는 OKR 화면"),
      },
      {
        title: translate("모든 일이 연결되고, 성과로 이어지는 과정이 보입니다."),
        description: translate("목표부터 프로젝트, 개별 할 일까지 하나로. 지금 내가 하는 일이 팀의 어떤 성과에 기여하는지 한눈에 확인하세요."),
        alt: translate("할 일에서 프로젝트, Initiative, 핵심결과와 목표까지 이어지는 업무 관계와 진행 상태"),
      },
      {
        title: translate("기록은 간편하게, 실행에 더 집중하세요."),
        description: translate("AI와 대화하며 업무를 정리하고, 내용을 확인한 뒤 등록하세요."),
        alt: translate("대화로 정리한 프로젝트 제안의 내용을 생성 전에 확인하는 화면"),
      },
      {
        title: translate("Slack 연결은 버튼 하나로. 반복되는 관리는 봇에게."),
        description: translate("데일리 스크럼부터 누락된 담당자·기한 확인, Task 변경 알림까지. Slack에서 봇을 불러 Task를 만들고, OKRI에서 프로젝트별 업무를 함께 관리하세요."),
        alt: translate("데일리·관리·업무 관리·Task 변동 알림 봇을 함께 보여주는 Slack 봇 목록"),
      },
    ],
  };
}
