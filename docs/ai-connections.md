...
기존 연결 문구와 `OKRPTR 연결이 완료되었습니다.` 안내는 유지합니다. 복사 실패가 표시되면 클립보드 권한을 확인하고 재시도하세요.

## Claude 웹·앱

1. **Claude에 연결**을 누릅니다. Claude의 공식 설정에 `OKRPTR`과 `https://okrptr.com/api/mcp`가 미리 입력됩니다.
2. 내용을 확인하고 OKRPTR 계정으로 로그인합니다.
3. 승인 화면의 **계정·워크스페이스·역할·권한**을 확인한 후 연결을 승인합니다. 취소하면 연결 키는 발급되지 않습니다.
4. Claude 대화에서 OKRPTR을 활성화하고 조회 도구로 확인합니다.

Claude Free는 커스텀 커넥터 1개 제한이 있습니다. 조직 정책에 따라 Claude Team/Enterprise 조직 소유자의 추가 설정이 필요할 수 있습니다. **OKRPTR 관리자와 Claude 조직 관리자는 별개**입니다. 수동 주소 복사와 조직 관리자용 링크는 보조 안내를 펼치면 나옵니다. 외부 계정 상태를 추측해 연결 버튼을 막지 않습니다.
...
```sh
claude mcp add --transport http --scope user okrptr https://okrptr.com/api/mcp
```

이후 Code의 `/mcp`에서 OKRPTR 인증을 선택합니다. 같은 이름이 이미 있으면 기존 설정을 확인하세요. 앱은 명령을 자동 실행하거나 기존 연결을 삭제하지 않습니다. 승인 화면의 `localhost` 또는 `127.0.0.1` 콜백이 자신의 컴퓨터에서 시작한 연결인지 확인하세요.
