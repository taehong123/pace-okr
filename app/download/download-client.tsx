...
export function DownloadClient() {
  ...
  useEffect(() => {
    let active = true;
    const syncStatus = () => { if (active) setStatus(window.__OKRPTR_INSTALL__?.status ?? "unavailable"); };
    queueMicrotask(() => {
      if (!active) return;
      setDetected(/Macintosh/i.test(navigator.userAgent) ? "mac" : /Windows/i.test(navigator.userAgent) ? "windows" : null);
      syncStatus();
    });
    window.addEventListener("okrptr:installchange", syncStatus);
    return () => {
      active = false;
      window.removeEventListener("okrptr:installchange", syncStatus);
    };
  }, []);
