...
const memberName = (editor: ProjectReviewEditor | null, id: string) => editor?.members.find((m) => m.id === id)?.displayName ?? t("사용할 수 없는 멤버");
const label = (editor: ProjectReviewEditor | null, key: string, fallback: string) => systemPropertyLabel(editor?.properties.find((p) => p.systemKey === key), t, fallback);
...
function propertyText(editor: ProjectReviewEditor | null, name: string, value: EditablePropertyValue) {
  ...
  return empty(value) ? t("미지정") : type === "member" ? memberName(editor, String(value))
    : type === "members" && Array.isArray(value) ? value.map((id) => memberName(editor, id)).join(", ")
    : typeof value === "boolean" ? t(value ? "체크됨" : "체크 안 됨") : typeof value === "number" ? value.toLocaleString(getClientLocale()) : type === "date" && typeof value === "string" ? displayDate(value) : String(value);
}
...
export function ReviewFields({ review, proposal: p, editor, errors, disabled, onChange }: {
  ...
  const origin = (key: string, value: unknown) => {
    ...
    return t(JSON.stringify(initial) !== JSON.stringify(value) ? "수정됨" : empty(value) ? "미지정" : review.fieldOrigins?.[key] === "default" ? "기본값" : "대화 초안");
  };
  ...
  function systemSelect(key: "status" | "priority", names: Record<string, string>) {
    ...
    return <select {...props(key)} value={p[key]} onChange={(e) => update(key, e.target.value as ProjectProposal[typeof key])}>
      {!options.includes(p[key]) && <option value={p[key]}>{t("현재 사용할 수 없는 값 · 다시 선택")}</option>}
      {options.map((value) => <option value={value} key={value}>{t(names[value])}</option>)}
    </select>;
  }
  return <section aria-labelledby="review-edit-heading">
    <h2 id="review-edit-heading">{t("Project 내용 수정")}</h2>
    <p className="review-help">{t("대화 초안·기본값·수정됨으로 구분합니다. 책임자와 기한은 미지정이어도 생성할 수 있습니다.")}</p>
    <div className="review-fields">
      {field("title", t("Project 제목 (필수)"), <input {...props("title")} value={p.title} maxLength={500} onChange={(e) => update("title", e.target.value)} />)}
      {field("description", t("설명 · 범위와 완료 기준"), <textarea {...props("description")} value={p.description} maxLength={20000} rows={4} onChange={(e) => update("description", e.target.value)} />)}
      ...
      {([ ["status", t("상태"), statusNames], ["priority", t("우선순위"), priorityNames] ] as const).map(([key, title, names]) => <div key={key}>{field(key, label(editor, key, title), systemSelect(key, names))}</div>)}
      ...
      {field("templateId", t("본문 템플릿"), <select {...props("templateId")} value={p.templateId ?? ""} onChange={(e) => update("templateId", e.target.value || null)}><option value="">{t("미지정")}</option>{p.templateId && !selectedTemplate && <option value={p.templateId}>{t("사용할 수 없는 템플릿 · 다시 선택")}</option>}{editor.templates.map((t) => <option value={t.id} key={t.id}>{t.name}</option>)}</select>)}
      {custom.map((property) => {
...
export function ReviewSummary({ review, proposal: p, editor, selection }: { review: ProjectReview; proposal: ProjectProposal; editor: ProjectReviewEditor | null; selection: InitiativeChoice | null }) {
  ...
  const rows: [string, string][] = [
    [label(editor, "project_dri", "책임자"), pending ? p.driMemberId ? memberName(editor, p.driMemberId) : t("미지정") : review.fieldLabels.dri || t("미지정")],
    [label(editor, "project_workers", "참여자"), (pending ? p.workerMemberIds.map((id) => memberName(editor, id)) : review.fieldLabels.workers).join(", ") || t("미지정")],
    [label(editor, "due_date", "기한"), p.dueDate ? displayDate(p.dueDate) : t("미지정")], [label(editor, "priority", "우선순위"), t(priorityNames[p.priority])],
    [`${label(editor, "status", "상태")} · ${label(editor, "progress", "진행률")}`, `${t(statusNames[p.status])} · ${p.progress.toLocaleString(getClientLocale())}%`],
    [t("본문 템플릿"), pending ? template?.name || t("미지정") : review.fieldLabels.template || t("미지정")],
    [t("연결 OKR 파일"), selection ? selection.cycleName ?? t("파일 없음") : t("Initiative 선택 후 결정")],
  ...
  return <section className="review-summary" aria-labelledby="project-summary-heading">
    <h2 id="project-summary-heading">{pending ? t("최종 생성 내용") : t("확인한 Project 내용")}</h2><h3>{p.title || t("제목 미입력")}</h3>
    <p className="review-description">{p.description || t("범위·완료 기준 미입력")}</p><p>{selection?.path.join(" → ") ?? t("아직 Initiative를 선택하지 않았습니다.")}</p>
    <dl>{rows.map(([name, value], index) => <div key={`${name}-${index}`}><dt>{name}</dt><dd>{value}</dd></div>)}</dl>
    {preview != null && <details><summary>{t("적용할 템플릿 본문 미리보기")}</summary><p className="review-help">{t("템플릿 뒤에 위의 범위·완료 기준이 이어서 저장됩니다.")}</p><p className="review-description">{preview || t("빈 본문")}</p>{preview.length === 4000 && <p className="review-help">{t("앞 4,000자만 표시했습니다.")}</p>}</details>}
  </section>;
}
