...
export function AiUsageMeter({ usage, compact = false, loading = false }: { usage: AiUsage | null; compact?: boolean; loading?: boolean }) {
  ...
  return <article className={`ai-usage-meter${compact ? " ai-usage-meter-compact" : ""}`} aria-label="AI 사용량">
    <header><span>{compact ? "이번 달 AI 사용량" : "AI 사용량"}</span><b>{value}</b></header>
    <div className="ai-usage-track" role="progressbar" aria-label="이번 달 AI 사용량" aria-valuemin={0} aria-valuemax={100} aria-valuenow={percent?.usedPercent} aria-valuetext={percent ? `${value}, ${remaining}` : value} aria-busy={loading || undefined}>
      {percent && <i style={{ width: `${percent.usedPercent}%` }} />}
    </div>
    {!loading && <p>{remaining}{percent && percent.usedPercent >= 100 ? " · 한도 도달" : ""}</p>}
  </article>;
}
...
export function ChatAiUsage({ scope, refreshKey }: { scope: AiUsageScope; refreshKey: number }) {
  ...
  return <div className="chat-ai-usage">
    ...
    {loaded && !result.usage && <button type="button" className="secondary" onClick={() => { invalidateAiUsage(); setAttempt((value) => value + 1); }}>사용량 다시 확인</button>}
  </div>;
}
