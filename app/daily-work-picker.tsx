...
export function DailyWorkPicker({ work, selected, disabled, noPlanned, onChange, onNoPlanned, onOpen }: {
  ...
  return <section className="daily-task-picker" aria-label={t("오늘 할 업무")}>
    <header><div><b>{t("오늘 할 업무")}</b></div><strong>{selected.length}/50</strong></header>
    <div className="daily-task-groups">{(["project", "task", "routine"] as const).map((kind) => {
      ...
      return entries.length ? <section key={kind}><h3>{{ project: "Project", task: "Task", routine: "Routine" }[kind]}</h3>{entries.map((entry) => {
        ...
        return <div className="daily-task-option" data-kind={entry.kind} key={entry.key}><label><input type="checkbox" aria-label={t("{value1} 선택", { value1: messageValue(entry.title) })} checked={checked} disabled={disabled || noPlanned || (!checked && selected.length >= 50)} onChange={() => onChange(checked ? selected.filter((key) => key !== entry.key) : [...selected, entry.key])} /><span><b>{entry.title}</b><small>{entry.parentTitle}{entry.dueDate ? ` · ${entry.dueDate}` : ""}</small></span></label><button type="button" aria-label={t("{value1} 열기", { value1: messageValue(entry.title) })} title={t("{value1} 열기", { value1: messageValue(entry.title) })} onClick={() => onOpen(entry)}><ChevronRight size={16} /></button></div>;
      })}</section> : null;
    })}</div>
    {!work.length && <p className="daily-empty">{t("배정된 미완료 업무가 없습니다.")}</p>}
    <label className="daily-none"><input type="checkbox" checked={noPlanned} disabled={disabled} onChange={(event) => onNoPlanned(event.target.checked)} />{t("오늘 예정 없음")}</label>
  </section>;
}
