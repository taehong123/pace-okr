...
export default function BillingView({ onNotice }: { onNotice: (message: string, tone?: NoticeTone) => void }) {
  ...
  return <section className="billing-page" aria-label="요금제 및 결제">
    ...
    <section className="billing-payment-section"><header><div><h3>결제수단과 자동 갱신</h3></div>{billing.paymentMethod && <div className="billing-card-chip"><CreditCard size={17} /><span><b>{billing.paymentMethod.cardCompany || "등록 카드"}</b><small>{billing.paymentMethod.maskedCard}</small></span></div>}</header>
      {!billing.canManage ? <p className="billing-member-note">현재 플랜과 사용량은 모든 멤버가 볼 수 있습니다. 카드·플랜·환불 관리는 워크스페이스 Owner에게 요청해 주세요.</p>
        : !billing.checkoutAvailable ? <div className="billing-setup-note"><LockKeyhole size={18} /><div><b>결제는 아직 활성화하지 않았습니다</b><p>Payple에 okrptr.com 정기결제·환불 승인을 받은 뒤 운영 보안값과 예약 청구 검증을 마치면 카드 등록 버튼이 표시됩니다. 그 전에는 Free 한도를 강제하지 않습니다.</p></div></div>
