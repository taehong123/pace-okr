...
export default function AIConnectionsDialog({ onNotice, onClose }: { onNotice: (message: string) => void; onClose: () => void }) {
  ...
  return <OverlayDialog title={t("AI 연결")} onRequestClose={onClose}>{(requestClose) => <section className="ai-connections">
    <header><div><h2>{t("AI 연결")}</h2><p>{t("내 AI에서 OKRI의 업무를 조회하고 관리하세요.")}</p></div><button type="button" className="icon-button" aria-label={t("AI 연결 닫기")} onClick={() => requestClose("close-button")}><X size={18} /></button></header>
    ...
    <div id={`${id}-panel`} role="tabpanel" aria-labelledby={`${id}-${tab}`} className="ai-connection-panel" tabIndex={0}>
      <div className="ai-connection-heading"><h3>{providerLabels[tab]}{t("에 OKRI 연결")}</h3><span className="ai-connection-state">{status}</span></div>
      {tab === "chatgpt" ? <>
        ...
        <ol className="ai-connection-steps"><li>{t("Claude에서 미리 입력된")}<b>OKRI</b> {t("이름과 주소 확인")}</li><li>{t("OKRI 로그인 후 계정·워크스페이스와 권한 승인")}</li><li>{tab === "claude_code" ? t("Claude Code의 /mcp에서 OKRI 확인") : t("Claude 대화에서 OKRI을 켜고 업무 조회")}</li></ol>
        {tab === "claude_code" && <>
