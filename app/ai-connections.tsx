...
export default function AIConnectionsDialog({ onNotice, onClose }: { onNotice: (message: string) => void; onClose: () => void }) {
  ...
  return <OverlayDialog title="AI 연결" onRequestClose={onClose}>{(requestClose) => <section className="ai-connections">
    <header><div><h2>AI 연결</h2><p>내 AI에서 OKRPTR의 업무를 조회하고 관리하세요.</p></div><button type="button" className="icon-button" aria-label="AI 연결 닫기" onClick={() => requestClose("close-button")}><X size={18} /></button></header>
    ...
    <div id={`${id}-panel`} role="tabpanel" aria-labelledby={`${id}-${tab}`} className="ai-connection-panel" tabIndex={0}>
      ...
      {tab === "chatgpt" ? <>
        ...
        <ol className="ai-connection-steps"><li>Claude에서 미리 입력된 <b>OKRPTR</b> 이름과 주소 확인</li><li>OKRPTR 로그인 후 계정·워크스페이스와 권한 승인</li><li>{tab === "claude_code" ? "Claude Code의 /mcp에서 OKRPTR 확인" : "Claude 대화에서 OKRPTR을 켜고 업무 조회"}</li></ol>
        {tab === "claude_code" && <>
