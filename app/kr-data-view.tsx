...
export default function KrDataView({ cacheKey, keyResults, cycles, readOnly, onProgressChange, onNotice }: {
  ...
  const [initialDraft, setInitialDraft] = useState<Draft>(() => emptyDraft(keyResults[0]?.id ?? ""));
  ...
  function openCreate(krId = keyResults.find((kr) => !byKr.has(kr.id))?.id ?? keyResults[0]?.id ?? "") {
    const nextDraft = emptyDraft(krId);
    setDraft(nextDraft);
  ...
  return <>
    ...
    {editing && <OverlayDialog title={editing === "new" ? "KR API 연결" : "KR API 설정"} variant="drawer" dirty={draftDirty} onRequestClose={() => setEditing(null)}>
