...
export async function POST(request: Request) {
  ...
  try {
    const payload = await request.json() as { date?: string; requestId?: string };
    const submission = await submitDailyDraft(authorization, payload.date ?? new Date().toISOString().slice(0, 10), "web", payload.requestId);
