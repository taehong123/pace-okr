...
export default function WorkspaceIdentity({ workspaceId, onNameChanged }: { workspaceId: string; onNameChanged: (name: string) => void }) {
  ...
  useEffect(() => {
    ...
    void fetch("/api/workspaces/profile", { cache: "no-store", headers: { "x-okri-workspace-id": workspaceId }, signal: controller.signal })
      .then(async (response) => {
  ...
  async function save(event: FormEvent, kind: "name" | "address" | "messageLanguage") {
    ...
    try {
      ...
      const response = await fetch("/api/workspaces/profile", { method: "PATCH", headers: { "Content-Type": "application/json", "x-okri-workspace-id": workspaceId }, body: JSON.stringify({ [kind]: value, revision: profile.revision }) });
      const data = await response.json() as { profile?: Profile; error?: string };
  ...
  return <section className="workspace-identity" aria-label={t("워크스페이스 이름과 주소")} aria-busy={loading || Boolean(busy)}>
    ...
    {profile && <>
      ...
      <form onSubmit={(event) => void save(event, "address")}>
        ...
        {profile.url && <div className="workspace-identity-link"><a href={profile.url}>{profile.subdomainsEnabled ? `${profile.address}.okri.ai` : t("워크스페이스 열기")}</a><button className="icon-button" type="button" onClick={() => void copyLink()} aria-label={copied ? t("링크 복사됨") : t("워크스페이스 링크 복사")} title={copied ? t("복사됨") : t("워크스페이스 링크 복사")}>{copied ? <Check size={16} /> : <Copy size={16} />}</button></div>}
      </form>
    </>}
